...
/**
 * Reload a gear item's image and AI description from a *specific* ASIN
 * the admin has entered, rather than a fuzzy brand/model search.
 *
 * - Image: pulled from Amazon for that exact ASIN (and HEAD-verified).
 *   If the ASIN doesn't resolve to itself on Amazon, no image is
 *   returned (`matched: false`) so we never swap in a different
 *   product's photo.
 * - Description: regenerated by the AI text model from the item's
 *   brand/name/model, grounded with the Amazon listing title when we
 *   found one.
 *
 * Intentionally NON-destructive: returns the proposed values for the
 * editor to populate so the admin can review the AI text before saving.
 * Nothing is written to the DB here.
 */
export async function reloadGearFromAsinAction(
  gearId: string,
  asinInput: string
) {
  const me = await getCurrentAppUser();
  ...
  // Shares the AI-enrichment rate-limit bucket — this makes one SerpAPI
  // call plus one AI text call.
  const rl = rateLimit(me.authId, "ai-gear-enrich", {
    maxRequests: 30,
    windowMs: 60_000,
  });
  ...
  // Look the ASIN up first so the description can be grounded with the
  // real Amazon listing title.
  let image;
  try {
